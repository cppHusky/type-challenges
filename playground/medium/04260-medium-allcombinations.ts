...
type StringToUnion<S extends string>=S extends `${infer A}${infer Rest}`
	...
	:''
type AllCombinations<S extends string,U extends string=StringToUnion<S>,V extends string=U>=U extends ''
	?''
	:`${U}${AllCombinations<Exclude<V,U>>}`|`${AllCombinations<Exclude<V,U>>}${U}`|`${AllCombinations<Exclude<V,U>>}`
